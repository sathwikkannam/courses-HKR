...
clearForm.addEventListener('change', () => {
    const selectedMode = document.getElementById('clear').value;

    fetch('/set_clear', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({clear: selectedMode})
    })
        .then(response => response.text())
        .then(data => alert(data)); // Display success or error message
});
